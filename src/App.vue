  ...
  <div class="overflow-x-hidden bg-[#0D2B33]">
    ...
    <div class=" w-full translate-y-[-21vw] py-[2vw]">
      ...
      <!-- list section -->
       ...
       <FoodItem  class="my-[1vw]" v-for="(item,index) in sortedItems" @click="chooseItem(index)" :key="index" :foodTitle="item.title" :foodPrice="item.price" :isSuggested="item.isSuggested" :imageUrl="item.imageUrl"/>
    </div>
  </div>
  <link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Saira:ital,wght@0,100..900;1,100..900&display=swap" rel="stylesheet">
<FoodExpanded :foodTitle="chosenItem.title" :isSuggested="chosenItem.isSuggested"  :imageUrl="chosenItem.imageUrl" :foodPrice="chosenItem.price" :description="chosenItem.description" v-if="isExpanded"/>
...
return {
  ...
  computed: {
    sortedItems() {
      return this.items
        .filter(item => item.imageUrl) // Exclude items with empty imageUrl
        .sort((a, b) => b.isSuggested - a.isSuggested) // Sort by isSuggested
        .concat(
          this.items.filter(item => !item.imageUrl) // Append items with empty imageUrl
        );
    },
    chosenItem(){
